...
void(*VisitFunc)(ALGraph G,int i); /* 函数变量(全局量) */
...
/* 对v的尚未访问的邻接点w递归调用DFS */
    ...
    VisitFunc(G,v);
    int w;
    for(w=FirstAdjVex(G,v);w>=0;w=NextAdjVex(G,G.vertices[v].data,G.vertices[w].data))
        if(!visited[w]) DFS(G,w);
}
void DFSTraverse(ALGraph G,void(*Visit)(ALGraph G,int i))
{ /* 对图G作深度优先遍历。算法7.4 */
